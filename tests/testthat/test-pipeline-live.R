# ==============================================================================
# LIVE Pipeline Tests for ctschooldata
# ==============================================================================
#
...
test_that("Connecticut DOE website is accessible", {

  skip_if_offline()
  skip("TODO: Add Connecticut DOE URL and verify HTTP 200")
})
...
test_that("Can download Connecticut enrollment data file", {
  skip_if_offline()
...
test_that("Can parse Connecticut enrollment file", {
  skip_if_offline()
...
test_that("ctschooldata data file has expected columns", {
...
test_that("get_raw_enr returns data for valid year", {
  ...
  # Get available years
  years_info <- ctschooldata::get_available_years()
  ...
  tryCatch({
    raw <- ctschooldata:::get_raw_enr(test_year)
...
test_that("get_available_years returns valid year range", {
  result <- ctschooldata::get_available_years()
...
test_that("fetch_enr returns data with no Inf or NaN", {
  ...
  tryCatch({
    ...
    data <- ctschooldata::fetch_enr(test_year, tidy = TRUE)
...
test_that("Enrollment counts are non-negative", {
  ...
  tryCatch({
    ...
    data <- ctschooldata::fetch_enr(test_year, tidy = FALSE)
...
test_that("tidy=TRUE and tidy=FALSE return consistent totals", {
  ...
  tryCatch({
    ...
    wide <- ctschooldata::fetch_enr(test_year, tidy = FALSE)
    tidy <- ctschooldata::fetch_enr(test_year, tidy = TRUE)
...
test_that("Cache functions exist and work", {
  ...
  tryCatch({
    path <- ctschooldata:::get_cache_path(2024, "enrollment")
