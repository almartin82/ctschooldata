---
title: "Getting Started with ctschooldata"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Getting Started with ctschooldata}
...
The `ctschooldata` package provides tools for accessing Connecticut public school enrollment data from the Connecticut State Department of Education (CSDE).
...
enr_2015 <- fetch_enr(2015, use_cache = TRUE)

# Fetch multiple years
enr_multi <- fetch_enr_multi(2010:2015, use_cache = TRUE)
```
...
For years 2007-2016, the package fetches actual enrollment counts from CTData.org. This data includes:
...
For years 2017 and later, the package falls back to the CT Open Data Education Directory. **Important limitations:**

- Contains **binary flags (0/1)** indicating whether a school offers a grade
...
For complete enrollment data (any year), you can manually export from EdSight:
...
3. Export to Excel
4. Import using `import_local_enr()`:
...
enr_2024 <- fetch_enr(2024, use_cache = TRUE)  # Uses cached data
```
...
|----------|-------------|
| `fetch_enr(end_year, use_cache = TRUE)` | Fetch enrollment for one year |
| `fetch_enr_multi(years, use_cache = TRUE)` | Fetch enrollment for multiple years |
| `import_local_enr(path, year)` | Import locally downloaded file |
| `get_available_years()` | Get range of available years |
...
| `cache_status()` | Show cached files |

## Year Convention

The package uses **end year** convention:
...
1. **No state-level aggregates**: The automated sources do not include state-wide totals. Calculate these manually if needed.

2. **Limited district coverage (2007-2016)**: CTData.org only includes approximately 6 sample districts, not all CT districts.

3. **Binary flags (2017+)**: The Education Directory only indicates grade offerings, not actual enrollment counts.

4. **No demographic breakdowns**: Race/ethnicity, ELL, and special education data requires manual EdSight export.
...
# Fetch data
enr = ct.fetch_enr(2015, use_cache = TRUE)
